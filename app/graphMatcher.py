...
def elementPrüfen(
    MGraph, SGraph, studentNode, studentData, fehler, fehler_visualisierung
):
    fehlerdict = {
        "richtige_Knoten": [],
        "falsche_Nachbarn": [],
    ...
    for musterNode, musterData in MGraph.nodes(data=True):
        ...
        if musterData["type"] == studentData["type"]:
            musterLabelListe = (
                musterData["label"]
                if isinstance(musterData["label"], list)
                else [musterData["label"]]
            )
            if any(label == studentData["label"] for label in musterLabelListe):
                ...
                nachbarVergleich = nachbarnPrüfen(
                    MGraph, musterNode, SGraph, studentNode
                )
                ...
                if nachbarVergleich:
                    ...
                    fehler["helper-liste-falscher-Name"].append(
                        f"{studentNode} gemeint: {musterNode}"
                    )
                else:
    ...
    if fehlerdict["richtige_Knoten"]:
        fehler_visualisierung["richtige_Knoten"].append(
            f"   style {studentNode} color:#000000,fill:#d4edda,stroke:#5a8f66,stroke-width:2px"
        )
        return
    elif fehlerdict["falsche_Nachbarn"]:
        fehler_visualisierung["falsche_Nachbarn_gelb"].append(
            f"   style {studentNode} color:#000000,fill:#FFD966,stroke:#FFD966,stroke-width:2px"
        )
        return
    elif fehlerdict["falscher_Name_Knoten"]:
        fehler_visualisierung["falscher_Name_Knoten_rot"].append(
            f"   style {studentNode} color:#CC0000,stroke-width:2px,font-weight:bold"
        )
        return
    elif fehlerdict["falscher_Typ_Knoten"]:
        fehler_visualisierung["falscher_Typ_Knoten_rot"].append(
            f"   style {studentNode} color:#000000,stroke:#CC0000,stroke-width:3px"
        )
    elif fehlerdict["falscher_Knoten"]:
        fehler_visualisierung["falscher_Knoten_rot"].append(
            f"   style {studentNode} fill:#F4CCCC,stroke:#CC0000,color:#000000,stroke-width:2px,font-weight:bold"
        )
        return


def kardinalitätPrüfen(MGraph, SGraph, studentNode, studentData, fehler_visualisierung):
    fehlerdict = {"richtigeKanten": [], "falscheKanten": []}
    for musterNode, musterData in MGraph.nodes(data=True):
        if (
            musterData["type"] == "Entität"
            and studentData["type"] == "Entität"
            or musterData["type"] == "Relationship"
            and studentData["type"] == "Relationship"
            or musterData["type"] == "Entität(Supertyp)"
            and studentData["type"] == "Entität(Supertyp)"
            or musterData["type"] == "Entität(Subtyp)"
            and studentData["type"] == "Entität(Subtyp)"
            or studentData["type"] == "Schwache Entität"
            and musterData["type"] == "Schwache Entität"
        ):
            studentenNachbarn = sorted(list(SGraph.neighbors(studentNode))) + sorted(
                list(SGraph.predecessors(studentNode))
            )
            musterLabels = [
                (
                    MGraph.nodes[n]["label"]
                    if isinstance(MGraph.nodes[n]["label"], list)
                    else [MGraph.nodes[n]["label"]]
                )
                for n in list(MGraph.neighbors(musterNode))
                + list(MGraph.predecessors(musterNode))
            ]
            ...
            for liste in musterNachbarn:
                for nachbar in liste:
                    if MGraph.has_edge(musterNode, nachbar) and SGraph.has_edge(
                        studentNode, nachbar
                    ):
                        musterEdgeData = MGraph.get_edge_data(musterNode, nachbar)
                        studentEdgeData = SGraph.get_edge_data(studentNode, nachbar)
                        if (
                            musterEdgeData.get("Kardinalität") is not None
                            and studentEdgeData.get("Kardinalität") is not None
                            and not any(
                                k in studentEdgeData.get("Kardinalität")
                                for k in musterEdgeData.get("Kardinalität")
                            )
                        ):
                            fehlerdict["falscheKanten"].append(
                                studentEdgeData.get("Nummer")
                            )
                        elif (
                            musterEdgeData.get("Kardinalität") is not None
                            and studentEdgeData.get("Kardinalität") is not None
                            ...
                            fehlerdict["richtigeKanten"].append(
                                studentEdgeData.get("Nummer")
                            )
                    elif MGraph.has_edge(nachbar, musterNode) and SGraph.has_edge(
                        nachbar, studentNode
                    ):
    ...
    if fehlerdict["richtigeKanten"]:
        for number in fehlerdict["richtigeKanten"]:
            if number not in fehler_visualisierung["richtige_Kanten_grün"]:
                fehler_visualisierung["richtige_Kanten_grün"].append(
                    f"   linkStyle {number} color:#2ca02c,stroke:#d4edda,stroke-width:4px;"
                )
    if fehlerdict["falscheKanten"]:
        for number in fehlerdict["falscheKanten"]:
            if (
                number not in fehler_visualisierung["falsche_Kanten_rot"]
                and number not in fehler_visualisierung["richtige_Kanten_grün"]
            ):
                fehler_visualisierung["falsche_Kanten_rot"].append(
                    f"   linkStyle {number} stroke:#d62728,stroke-width:4px,color:#d62728,fill:none;"
                )
...
def entitätenPrüfen(
    ...
    for nachbar in studentenNachbarn:
        ...
        if (
            SGraph.nodes[nachbar]["type"] == "Entität"
            or SGraph.nodes[nachbar]["type"] == "Schwache Entität"
        ):  # schwache Entitäten sind über ein Relationship gebunden
            booleanEntitäten = True
        if (
            SGraph.nodes[studentNode]["type"] == "Entität(Subtyp)"
            and SGraph.nodes[nachbar]["type"] == "Entität(Supertyp)"
        ):
            anzahlSupertypen += 1
    if (
        anzahlPrimärschlüssel == 1
        and booleanEntitäten == False
        and (anzahlSupertypen == 0 or anzahlSupertypen == 1)
    ):
        elementPrüfen(
            MGraph, SGraph, studentNode, studentData, fehler, fehler_visualisierung
        )
        ...
        fehler_visualisierung["NichteinhaltungERM-Regeln_Info"].append(
            f"   style {studentNode} fill:#FFD966,stroke:#FFA500,stroke-width:2px"
        )  ########## HINZUFÜGEN


def relationshipsPrüfen(
    MGraph, SGraph, studentNode, studentData, fehler, fehler_visualisierung
):
    ...
    anzahlZusammengesetzteAttribute = (
        0  # es darf kein zusammengesetztes attribut besitzen
    )
    booleanRelationships = (
        False  # es darf keine verbindung zu anderen relationships haben
    )
    for nachbar in studentenNachbarn:
        if SGraph.nodes[nachbar]["type"] == "Primärschlüssel-Attribut":
            anzahlPrimärschlüssel += 1
    ...
    if (
        anzahlPrimärschlüssel == 0
        and anzahlZusammengesetzteAttribute == 0
        and booleanRelationships == False
    ):
...
def attributePrüfen(
    ...
    if (
        studentData["type"] == "Attribut"
        or studentData["type"] == "mehrwertiges Attribut"
    ):
        ...
        for nachbar in studentenNachbarn:
            if (
                len(studentenNachbarn) == 1
                and SGraph.nodes[nachbar]["type"] != "Attribut"
                and SGraph.nodes[nachbar]["type"] != "mehrwertiges Attribut"
                and SGraph.nodes[nachbar]["type"] != "Primärschlüssel-Attribut"
            ):
                # elementPrüfen(MGraph, musterNode, musterData, SGraph, studentNode, studentData)
                elementPrüfen(
                    MGraph,
        ...
        for nachbar in studentenNachbarn:
            if (
                SGraph.nodes[nachbar]["type"] == "Entität"
                or SGraph.nodes[nachbar]["type"] == "Schwache Entität"
                or SGraph.nodes[nachbar]["type"] == "Entität(Supertyp)"
                or SGraph.nodes[nachbar]["type"] == "Entität(Subtyp)"
                or SGraph.nodes[nachbar]["type"] == "zusammengesetztes Attribut"
                and len(studentenNachbarn) == 1
            ):
                # elementPrüfen(MGraph, musterNode, musterData, SGraph, studentNode, studentData)
                elementPrüfen(
                    MGraph,
...
def nachbarnPrüfen(MGraph, musterNode, SGraph, studentNode):
    ...
    for liste in musterNachbarn:
        nachbarVergleich = set(map(str, studentenNachbarn)) - set(
            map(str, liste)
        )  # sind alle Elemente aus Studentenlösung auch in Muster
        fehlendeElemente = set(map(str, liste)) - set(
            map(str, studentenNachbarn)
        )  # Fehlen Elemente aus Muster un Studentenlösung
